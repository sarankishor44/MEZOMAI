...
export default function AvatarFace({ size = 60, showGlow = true, gender = 'female', avatarState = 'listening', avatarStyle = 'gold' }) {
  ...
  const accent = STYLE_COLORS[avatarStyle] || STYLE_COLORS.gold
  ...
  const cssVars = useMemo(() => ({
    ...
    '--mouth-open': avatarState === 'talking' ? `${(12 + talkFrame * 16) * (size/120)}px` : gender === 'female' ? `${11 * (size/120)}px` : `${8 * (size/120)}px`,
    '--mouth-radius': avatarState === 'talking' ? `${(16 + talkFrame * 18) * (size/120)}px` : '999px',
    '--eye-scale': blink ? 0.08 : 1,
  }), [accent, avatarState, blink, size, stateColor, talkFrame, gender])

  return (
    <div
      className={`mez-avatar mez-avatar-${gender} mez-avatar-${avatarState}`}
      style={cssVars}
      aria-label={`MEZOMAI ${gender} AI avatar`}
      role="img"
    >
      ...
      <div className="mez-avatar-card">
        ...
        <Hair gender={gender} />
        <div className="mez-avatar-label">Admin</div>
      </div>
...
.mez-avatar-label {
  ...
  font-size: calc(var(--avatar-size) * .15);
  font-weight: 800;
  letter-spacing: 0;
...
@keyframes mezAvatarTalk {
  ...
  to { transform: translateY(calc(var(--avatar-size) * .02)); }
}
`
